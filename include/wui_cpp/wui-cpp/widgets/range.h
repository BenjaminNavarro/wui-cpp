...
public:
		...
		max(max)
	{
		...
		setter = [this](const wui::property& p){
					 internal_state_ = std::make_pair(static_cast<ValueT>(internal::readPropertyAsDouble(p, "lower_bound")), static_cast<ValueT>(internal::readPropertyAsDouble(p, "upper_bound")));
				 };
